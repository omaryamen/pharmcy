...
class VerificationService:
    ...
    def verify_email(self, *, user: User, code: str, request=None) -> User:
        ...
        # Attempt bookkeeping must survive the error response, so it runs
        # outside the success-path transaction.
    ...
    def reset_password(
        ...
        *,
        ...
        # Attempt bookkeeping must survive the error response.
        self._assert_usable_token(token, code)
        ...
        with transaction.atomic():
            ...
            record_event(
                self.events, user=user, event_type=SecurityEventType.PASSWORD_RESET_CONFIRMED, request=request
            )
        return user
